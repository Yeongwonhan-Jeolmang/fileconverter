    ...
    def convert(self, job: ConversionJob, progress_cb: ProgressCallback = None) -> ConversionResult:
        def _do() -> None:
            ...
            try:
                ...
                raise MissingDependencyError("pydub", "Install with: pip install pydub") from exc
            ...
            segment = AudioSegment.from_file(job.source_path, format=audio_format)

            sample_rate = options.get("sample_rate")
            if sample_rate:
                segment = segment.set_frame_rate(int(sample_rate)) # type: ignore

            channels = options.get("channels")
            if channels:
                segment = segment.set_channels(int(channels)) # type: ignore

            gain_db = options.get("gain_db")
            if gain_db:
                segment = segment.apply_gain(float(gain_db)) # type: ignore
            ...
            segment.export(job.output_path, format=export_format, **export_kwargs) # type: ignore
